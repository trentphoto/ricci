/* Runs in <head>, before analytics or paint. Two fixed, equal allocations of the
 * SAME offer — only the meatball wording differs. Preview either page with
 * ?preview=1. Local/non-production hosts are previews. Deploy the CRM endpoint
 * before publishing these pages. See docs/meatball-framing-test.md.
 */
(function () {
  "use strict";
  var experiment = "meatball-framing-v1";
  var variants = ["control", "free"];
  var paths = {
  ...
  window.RicciMeatballExperiment = context;
  if (preview || bot || variants.indexOf(current) === -1) return;

  var key = "ricci_meatball_v1";
  var saved;
  try {
