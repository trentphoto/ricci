...
| `catering-modal.js` | Builds the catering-order modal (tray menu, quantities, live total). Catering is **prepaid**: on submit, POSTs cart lines (ids + qty only, no prices) to `/api/catering` with `source: "catering-modal"`, which prices server-side (`src/catering-catalog.ts`) and returns a Square-hosted checkout URL; the browser redirects there. The order isn't real until Square reports the charge — same model as the pepperoni pre-order. Opened via `window.RicciCatering.open()` or any `[data-open-catering]` element. Collects name, **email (required)**, **phone (required)**, pickup date/time, prep, notes. |
